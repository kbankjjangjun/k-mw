...
function getStyles<Key extends string>(
  ...
  if (Array.isArray(styles)) {
    return styles
      ...
      .reduce<Record<string, CSSObject>>(
        (acc, item) => assignAccStyles(acc, item),
        {}
      )
  }
...
function getContextVariation({
  ctx,
  theme,
  params,
  variant,
  size,
}: GetContextVariations) {
  return ctx.reduce<Record<string, CSSObject>>((acc, item) => {
    if (item.variants && variant in item.variants) {
      assignAccStyles(
        acc,
        item.variants[variant](theme, params, { variant, size })
      )
    }
...
>(
  input:
    | ((theme: MantineTheme, params: Params, variations: Variations) => Input)
    | Input
) {
